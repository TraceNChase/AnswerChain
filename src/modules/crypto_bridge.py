...
def argon2id_batch(passwords: List[bytes], salts: List[bytes], m_cost: int, t: int, p: int, dk_len: int) -> List[bytes]:
    if len(passwords) != len(salts):
        raise ValueError("passwords and salts length mismatch")
    items = [{
        "password_b64": _b64u(pw),
        "salt_b64": _b64u(sa)
    ...
    keys_b64 = resp.get("keys_b64", [])
    return [_unb64u(k) for k in keys_b64]
