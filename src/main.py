...
NEW (this update):
- Distribution-Transforming Encoder (DTE) wrapper for the protected secret to reduce
  distinguishers under invalid keys (honey-encryption style behavior).
...
- Hardened constant-time comparisons and uniform error/IO behavior during recovery paths.
...
from modules.sss_bridge import sss_split, sss_combine, sss_combine_batch
...
from modules.crypto_bridge import argon2id_batch
...
class SimpleSecretDTE:
    """
    Lightweight DTE wrapper to reduce distinguishers for the protected secret.
    ...
    def encode(self, secret_text: str) -> dict:
        ...
        # Meta binds exact length and checksum. To allow exact offline recovery
        # (post-SSS), we also embed the plaintext as base64 inside the DTE meta.
        # This meta lives INSIDE the SSS-protected payload (not outer JSON).
        meta = {
            "v": 2,
            "len": len(secret_text),
            "chk": hashlib.sha3_256(secret_text.encode("utf-8")).hexdigest()[:16],
            "plain_b64": base64.b64encode(secret_text.encode("utf-8")).decode("ascii"),
        }
    ...
    def decode(self, seed_b64: str) -> str:
        ...
        try:
            ...
            # Prefer exact embedded plaintext when present (v2+)
...
def _decrypt_share_from_entry(entry: dict,
                              ...
                              pre_key: bytes | None = None) -> bytes | None:
    """
    ...
    try:
        ...
        key = pre_key if pre_key is not None else _derive_answer_key(alt_text, salt, t, m, p)
...
# ---- Argon2 runtime sanity (avoid build-time crashes) ------------------------
def _ensure_working_argon2_params(t: int, m: int, p: int) -> tuple[int, int, int]:
    """
    Validate Argon2id parameters by attempting a short derivation. If it fails
    (e.g., memory too low for the backend), bump memory cost progressively to
    safe levels. Returns a tuple of (t, m, p) that is known to work.
    """
    salt = random_bytes(16)
    pwd = "SECQ_param_check"
    tries = [m]
    # Reasonable bumps to satisfy library constraints
    for bump in (8192, 16384, 32768, 65536):
        if bump > m:
            tries.append(bump)
    for mv in tries:
        try:
            ...
            if mv != m:
                print(f"[INFO] Adjusted Argon2 memory_cost from {m} KiB to {mv} KiB for compatibility.")
                log_debug("Argon2 params adjusted for compatibility.", level="INFO", component="CRYPTO",
                          details={"old_m": m, "new_m": mv, "time_cost": t, "parallelism": p})
            return (t, mv, p)
        except Exception as e:
            log_exception(e, f"Argon2 param check failed for m={mv}")
            continue
    # Final fallback — keep original but warn
    print("[WARNING] Argon2 parameters may be unsupported; proceeding anyway. Build may fail.")
    return (t, m, p)
...
def prompt_pad_size_multi(max_b64_len: int) -> int:
    """
    Ask user for PAD size with a hard lower bound of (2 + max_base64_length),
    since SSS packing prepends a 2-byte length header before the secret bytes.
    """
    min_pad = max_b64_len + 2
    recommended_pad = max(128, _bucketize_pad_size(min_pad + 30))
    user_pad = recommended_pad
    print(f"\nCustom PAD size? Press ENTER to use recommended={recommended_pad}.")
    try_pad_str = safe_input(f"PAD must be >= {min_pad} (2-byte header + max Base64 length): ", "").strip()
    if try_pad_str:
        try:
            user_pad_input = int(try_pad_str)
            if user_pad_input < min_pad:
                print(f"Provided pad < required minimum. Forcing {min_pad} instead.\n")
                user_pad = min_pad
            else:
    ...
    if user_pad < min_pad:
        user_pad = _bucketize_pad_size(min_pad)
        print(f"Corrected final pad to {user_pad} to meet minimum.\n")
    log_debug(f"Using PAD size (bucketized): {user_pad}", level="INFO", component="CRYPTO",
              details={"min_pad": min_pad, "max_b64_len": max_b64_len})
    return user_pad
...
def save_questions(questions):
    ...
    def _prompt_base_name(prompt: str) -> str:
        while True:
            ...
            if any(ch in name for ch in '\\/:*?"<>|'):
                print("Name contains invalid path characters.")
                continue
            return name
    ...
    log_debug(
        ...
        level="INFO",
        component="CRYPTO",
        details={"real_len": len(real_b64)}
    )
    pad_size = prompt_pad_size_multi(max_b64_len)

    # Argon2 parameters
    arg_time, arg_mem, arg_par = prompt_argon2_parameters()
    # Ensure Argon2 params actually work with the backend; bump if needed
    arg_time, arg_mem, arg_par = _ensure_working_argon2_params(arg_time, arg_mem, arg_par)
    ...
    # Split real-only shares across correct alternatives; provide dummy shares for incorrect picks
    # Try with selected pad; if it fails, auto-bump to a safer bucket and retry once.
    real_shares_correct = None
    shares_real_count = sum(1 for it in all_items if it[4])
    # Build a robust list of pad attempts: user pad, min bucket, and larger safe buckets
    min_pad_required = 2 + len(real_b64)
    pads = []
    def _add_pad(v):
        if v not in pads:
            pads.append(v)
    _add_pad(pad_size)
    _add_pad(max(128, _bucketize_pad_size(min_pad_required)))
    _add_pad(_bucketize_pad_size(min_pad_required + 32))
    _add_pad(256)
    _add_pad(384)
    _add_pad(512)
    pad_attempts = [p for p in pads if p is not None]
    for idx_attempt, pad_try in enumerate(pad_attempts, start=1):
        try:
            print(f"[INFO] Attempting REAL split with PAD={pad_try} (attempt {idx_attempt}/{len(pad_attempts)})")
            log_debug("Attempting sss_split for REAL path.", level="INFO", component="CRYPTO",
                      details={"attempt": idx_attempt, "pad": pad_try, "shares": shares_real_count, "threshold": r_thr,
                               "seed_b64_len": len(real_b64)})
            real_shares_correct = asyncio.run(
                sss_split(real_b64.encode("utf-8"), shares_real_count, r_thr, pad=pad_try)
            )
            pad_size = pad_try  # update to the effective pad used
            break
        except Exception as e:
            log_exception(e, f"REAL sss_split attempt {idx_attempt} failed")
            real_shares_correct = None
    if real_shares_correct is None:
        # Could not build shares even after retry → offer questions-only save
        print("\n[ERROR] Could not build the encrypted kit (REAL path). Details were logged.")
        print("Internal Decryption Test (beta): skipped (kit not built)")
        print("You can still save ONLY your security questions now and try building the kit later.")
        try:
            choice = input("Save questions only? (y/N): ").strip().lower()
        except Exception:
            choice = 'n'
        if choice == 'y':
            try:
                SAVE_DIR.mkdir(parents=True, exist_ok=True)
                while True:
                    base_name = _prompt_base_name("Enter a base name for the questions files: ")
                    json_path = SAVE_DIR / f"{base_name}.json"
                    txt_path  = SAVE_DIR / f"{base_name}.txt"
                    if not json_path.exists() and not txt_path.exists():
                        break
                    overwrite = input("Files exist. Overwrite? (y/N): ").strip().lower()
                    if overwrite == 'y':
                        break
                    print("Please choose a different name.")
                # Save the raw questions structure as captured in this session
                with open(json_path, 'w', encoding='utf-8') as jf:
                    json.dump(questions, jf, indent=2)
                with open(txt_path, 'w', encoding='utf-8') as tf:
                    tf.write("--- QUESTIONS (plaintext snapshot) ---\n\n")
                    for qd in questions:
                        qtype = "CRITICAL" if qd.get("is_critical") else "STANDARD"
                        tf.write(f"[Question {qd['id']}] {qd['text']} (Type: {qtype})\n")
                        for i, alt in enumerate(qd.get('alternatives', []), 1):
                            letter = chr(ord('A')+i-1)
                            tf.write(f" {letter}) {alt}\n")
                        tf.write(f" Correct: {', '.join(qd.get('correct_answers', []))}\n\n")
                print("[SAVED] Questions written:")
                print(f"  JSON: {json_path}")
                print(f"  TXT : {txt_path}")
            except Exception as e2:
                log_exception(e2, "Failed to persist questions snapshot")
        try:
    ...
    auth_catalog = [_auth_entry(real_b64)]
    final_auth = auth_catalog[0]
    ...
    def _enc_one_share(plaintext_share: bytes, q_hash: str, alt_text: str, alg_choice: str) -> dict:
        ...
        try:
            ...
            # As a last resort, try with a safer memory setting
            safe_mem = max(arg_mem, 16384)
            key = _derive_answer_key(alt_text, salt, arg_time, safe_mem, arg_par)
            log_debug("Per-share Argon2 derivation retried with safer memory.", level="WARNING", component="CRYPTO",
                      details={"original_m": arg_mem, "used_m": safe_mem})
    ...
    for global_idx, (q_hash, a_hash, q_text, alt_text, is_corr) in enumerate(all_items):
        ...
        log_debug("Mapped Q/A to encrypted shares.",
                  level="INFO", component="CRYPTO",
    ...
    recovery_kit = {
        "config": {
            ...
            "version": KIT_VERSION, "secrets_count": 1,
    ...
    # ---------------- Internal Decryption Test (Beta / Verbose Logging) -----------------
    # Purpose: Immediately verify that the kit can be decrypted using the known correct
    # answers in memory. This helps catch mismatches between encryption and recovery.
    print("\n--- Internal Decryption Test (beta) ---")
    try:
        cfg_test = recovery_kit.get("config", {})
        enc_shares_test = recovery_kit.get("encrypted_shares", {})
        r_thr_test = int(cfg_test.get("real_threshold", r_thr))
        version_test = int(cfg_test.get("version", KIT_VERSION))

        # Collect decrypted shares for the REAL path (s0) using per-entry KDF
        dec_partials: list[bytes] = []
        dec_meta: list[dict] = []
        total_correct_expected = 0
        for q in questions:  # use original in-memory questions with correct_answers
            q_text = q.get("text", "")
            alts = list(q.get("alternatives", []))
            corrects = list(q.get("correct_answers", []))
            total_correct_expected += len(corrects)
            q_hash = _integrity_hash_for_kit(q_text, alts)
            q_block = enc_shares_test.get(q_hash) or {}
            for alt in corrects:
                a_hash = _alt_hash_for_kit(alt)
                sblock = q_block.get(a_hash) or {}
                entry = sblock.get("s0")
                if not entry:
                    log_error("Self-test: missing s0 entry for correct answer.", details={"q_hash": q_hash, "a_hash": a_hash, "alt": alt})
                    continue
                try:
                    salt_b64 = entry.get("salt") or entry.get("salt_b64")
                    salt = base64.b64decode(salt_b64)
                    kdf = entry.get("kdf") or {}
                    tt = int(kdf.get("t", arg_time))
                    mm = int(kdf.get("m", arg_mem))
                    pp = int(kdf.get("p", arg_par))
                    key = _derive_answer_key(alt, salt, tt, mm, pp)
                    alg = (entry.get("algorithm") or "aes256gcm")
                    aad = _aad_bytes(q_hash, a_hash, alg or "aes256gcm", version_test)
                    pt = _aead_decrypt(alg or "aes256gcm", entry, key, aad=aad)
                    dec_partials.append(pt)
                    dec_meta.append({
                        "q_text": q_text,
                        "alt": alt,
                        "q_hash": q_hash,
                        ...
                        "kdf": {"t": tt, "m": mm, "p": pp},
                        "share_len": len(pt),
                        "x_index": pt[-1] if pt else None,
                        "share_sha3_256": hash_share(pt)
                    })
                except Exception as e:
                    log_exception(e, "Self-test: decryption failed for a correct answer.")
                    continue

        # Deduplicate by (length, x-index)
        uniq_map = {}
        for i, s in enumerate(dec_partials):
            key = (len(s), s[-1] if s else None)
            if key not in uniq_map:
                uniq_map[key] = s
        uniq_shares = list(uniq_map.values())
        log_debug("Self-test: decrypt summary.", level="INFO", component="CRYPTO",
                  details={"decrypted_total": len(dec_partials), "unique_by_x": len(uniq_shares),
                           "threshold": r_thr_test, "expected_correct_total": total_correct_expected})

        # Try sliding windows first, then random sampling if needed
        import random
        def _combine_try(shs: list[bytes]) -> bytes | None:
            if len(shs) < r_thr_test:
                return None
            # Sliding windows
            for start in range(0, max(0, len(shs) - r_thr_test + 1)):
                try:
                    cb = asyncio.run(sss_combine(shs[start:start + r_thr_test]))
                    return cb
                except Exception:
                    continue
            # Light random sampling (up to 200 tries)
            for _ in range(200):
                idxs = sorted(random.sample(range(len(shs)), r_thr_test))
                try:
                    cb = asyncio.run(sss_combine([shs[i] for i in idxs]))
                    return cb
                except Exception:
                    continue
            return None

        combined_bytes = _combine_try(uniq_shares)
        test_pass = False
        rec_b64_val: str | None = None
        if combined_bytes is not None:
            # Robust Base64 extraction (handles header+pad and raw forms)
            rec_b64 = _extract_valid_b64_from_combined(combined_bytes)
            if not rec_b64:
                log_error("Self-test: combined bytes not parseable as Base64.",
                          details={"combined_len": len(combined_bytes)})
            else:
                # Base64 strict decode + AUTH
                try:
                    seed_bytes = base64.b64decode(rec_b64.encode("utf-8"), validate=True)
                    matched = False
                    # final_auth first
                    try:
                        fa = cfg_test.get("final_auth") or {}
                        salt = base64.b64decode(fa.get("salt", "")) if fa else b""
                        exp = base64.b64decode(fa.get("hmac_sha256", "")) if fa else b""
                        if salt and exp:
                            k = hkdf_sha256(seed_bytes, salt, b"SECQ final-auth v3", 32)
                            tag = hmac_sha256(k, seed_bytes)
                            matched = consttime_equal(tag, exp)
                    except Exception:
                        matched = False
                    # catalog if needed
                    if not matched:
                        for e in (cfg_test.get("auth_catalog") or []):
                            try:
                                salt = base64.b64decode(e.get("salt", ""))
                                exp = base64.b64decode(e.get("hmac_sha256", ""))
                                k = hkdf_sha256(seed_bytes, salt, b"SECQ final-auth v3", 32)
                                tag = hmac_sha256(k, seed_bytes)
                                if consttime_equal(tag, exp):
                                    matched = True
                                    break
                            except Exception:
                                continue
                    test_pass = bool(matched)
                    if matched:
                        rec_b64_val = rec_b64
                    log_debug("Self-test: combine/auth result.", level="INFO", component="CRYPTO",
                              details={"combined_len": len(combined_bytes),
                                       "recovered_b64_len": len(rec_b64),
                                       "auth_ok": matched})
                except Exception as e:
                    log_exception(e, "Self-test: Base64/auth failed.")
        else:
            log_error("Self-test: unable to combine shares (no valid T-subset).",
                      details={"unique_shares": len(uniq_shares), "threshold": r_thr_test})

        print(f"Decryption test of security questions: {'Passed' if test_pass else 'Failed'}")
        # Optional human-readable proof without leaking in logs
        try:
            if test_pass and rec_b64_val is not None:
                same_seed = (rec_b64_val == real_b64)
                packed = base64.b64decode(rec_b64_val.encode('utf-8'))
                meta_bytes = packed[32:]
                meta = json.loads(meta_bytes.decode('utf-8')) if meta_bytes else {}
                print("[Proof] Recovered REAL seed matches builder seed:", "YES" if same_seed else "NO")
                print(f"[Proof] REAL meta: len={meta.get('len')}, chk={str(meta.get('chk'))[:8]}…")
                # Offer optional plaintext reveal (Unsafe Diagnostic Mode)
                try:
                    ans = input("Reveal plaintext secrets you entered (local diag)? (y/N): ").strip().lower()
                except Exception:
                    ans = 'n'
                if ans == 'y':
                    print(f"REAL SECRET: {real_secret}")
        except Exception:
            pass
    except Exception as e:
        log_exception(e, "Self-test: unexpected error")
        print("Decryption test of security questions: Failed")
    # ---------------- End Internal Decryption Test --------------------------------------
    ...
    # Ensure prompt is always visible and not skipped.
    try:
        import sys
        sys.stdout.flush()
    except Exception:
        pass
    decision = None
    while decision not in ('j', 'c'):
        decision = prompt_save_decision()
    if decision == 'j':
        SAVE_DIR.mkdir(parents=True, exist_ok=True)
        while True:
            base_name = _prompt_base_name("Enter a base name for the save files: ")
        ...
        with open(txt_file, "w", encoding="utf-8") as tf:
            ...
            tf.write(f"\nAuth Catalog Entries: {len(auth_catalog)}\n")
            tf.write("\n--- SECURITY QUESTIONS ---\n\n")
            for q in questions:
...
def _try_combine_with_sampling(partials: list[bytes], r_thr: int) -> bytes | None:
    ...
    if n == r_thr:
        try:
            out = asyncio.run(sss_combine(partials))
            return out
        except Exception:
            return None
    max_exhaustive = 5000
    ...
    if total_combos <= max_exhaustive:
        for idxs in combinations(range(n), r_thr):
            try:
                out = asyncio.run(sss_combine([partials[i] for i in idxs]))
                return out
            except Exception:
                continue
        return None
    ...
    seen = set()
    # Increase sampling attempts substantially to improve success probability
    for _ in range(5000):
...
# --- Validated reconstruction helpers ----------------------------------------
def _extract_valid_b64_from_combined(combined_bytes: bytes) -> str | None:
    """
    Try to parse the combined SSS output as a valid Base64 string.
    Handles both direct text and padded buffers (2-byte length header + NULs).
    Returns the Base64 string if valid, otherwise None.
    """
    # Primary expectation: a UTF-8 base64 string
    try:
        ...
        pass
    # Fallback: strip possible 2-byte header and trailing NUL padding
    try:
...
def _auth_ok_for_entry(seed_bytes: bytes, entry: dict) -> bool:
    try:
        salt = base64.b64decode(entry.get("salt", ""))
        expected = base64.b64decode(entry.get("hmac_sha256", ""))
        k_auth = hkdf_sha256(ikm=seed_bytes, salt=salt, info=b"SECQ final-auth v3", dk_len=32)
        calc = hmac_sha256(k_auth, seed_bytes)
        return consttime_equal(calc, expected)
    except Exception:
        return False

def _try_combine_validated(
    partials: list[bytes],
    r_thr: int,
    final_auth_entry: dict | None,
    auth_catalog: list[dict] | None,
    max_attempts: int = 5000,
    prefer_final_only: bool = True,
) -> tuple[str, str] | None:
    """
    Attempts to reconstruct a valid, authenticated Base64 seed string from the provided partial shares.
    - Tries deterministic/exhaustive search when small; otherwise random sampling up to max_attempts.
    - If prefer_final_only is True, only accepts candidates that authenticate against final_auth_entry.
      Otherwise, accepts any candidate that authenticates against any entry in auth_catalog.
    Returns (variant, recovered_b64) where variant is 'REAL' or 'DECOY', or None if not found.
    """
    n = len(partials)
    if n < r_thr:
        return None

    def validate_candidate(cb: bytes) -> tuple[bool, str]:
        b64 = _extract_valid_b64_from_combined(cb)
        if not b64:
            return (False, "")
        try:
            seed_b = base64.b64decode(b64.encode("utf-8"), validate=True)
        except Exception:
            return (False, "")
        if prefer_final_only and final_auth_entry is not None:
            return ((True, b64) if _auth_ok_for_entry(seed_b, final_auth_entry) else (False, ""))
        if auth_catalog:
            for e in auth_catalog:
                if _auth_ok_for_entry(seed_b, e):
                    return (True, b64)
        return (False, "")
    ...
    if n == r_thr:
        try:
            cb = asyncio.run(sss_combine(partials))
            ok, b64 = validate_candidate(cb)
            if ok:
                return ("REAL" if prefer_final_only else "DECOY", b64)
        except Exception:
            pass
        return None

    # Exhaustive when manageable
    total_combos = math.comb(n, r_thr) if hasattr(math, "comb") else float("inf")
    if total_combos <= 5000:
        sets = [tuple(idxs) for idxs in combinations(range(n), r_thr)]
        # Batch combine
        results = asyncio.run(sss_combine_batch(partials, sets))
        for cb in results:
            if cb is None:
                continue
            ok, b64 = validate_candidate(cb)
    ...
    def flush_batch() -> tuple[bool, str]:
        if not batch:
            return (False, "")
        results = asyncio.run(sss_combine_batch(partials, batch))
        for cb in results:
            if cb is None:
                continue
            ok, b64 = validate_candidate(cb)
    ...
    for _ in range(max_attempts):
        idxs = tuple(sorted(pysecrets.randbelow(n) for _ in range(r_thr)))
        if idxs in seen:
            continue
        seen.add(idxs)
        batch.append(idxs)
    ...
    if ok:
        return ("REAL" if prefer_final_only else "DECOY", b64)
    # Deterministic sliding window as a last resort
    if n >= r_thr:
        for start in range(0, n - r_thr + 1):
            try:
                cb = asyncio.run(sss_combine(partials[start:start + r_thr]))
                ok, b64 = validate_candidate(cb)
                if ok:
                    return ("REAL" if prefer_final_only else "DECOY", b64)
            except Exception:
...
def run_recovery_kit_flow(kit: dict, kit_path: Path):
    ...
    entries_s0: list[tuple[dict, str, bytes, str, str]] = []  # (entry, alt_text, salt_bytes, q_hash, a_hash)
    total_selected = 0
    total_alternatives_in_kit = 0
    for item in chosen:
        qobj = item["q"]
        picks = item["picks"]
        q_text = qobj.get("text", "")
        alts = qobj.get("alternatives", [])
        total_alternatives_in_kit += len(alts)
        ...
        for alt in picks:
            ...
            # Defer actual decrypt to batch KDF step
            try:
                salt_b64 = entry.get("salt") or entry.get("salt_b64")
                salt = base64.b64decode(salt_b64)
                entries_s0.append((entry, alt, salt, q_hash, alt_hash))
            except Exception as e:
                log_exception(e, "Invalid salt for entry")
        total_selected += max(0, len(picks))

    if total_selected < r_thr:
        log_error("Insufficient selections to meet threshold.", details={"selected": total_selected, "threshold": r_thr})
        print(f"\nYou selected {total_selected} alternative(s) in total, but threshold is {r_thr}.")
        print("Please re-run recovery and select at least the threshold number of alternatives across questions.")
        return

    # Batch-derive per-answer keys (Argon2id) for REAL path s0 entries
    try:
        if entries_s0:
            pw_list = [ _norm_for_kit(alt).encode('utf-8') for (_, alt, _, _, _) in entries_s0 ]
            salt_list = [ salt for (_, _, salt, _, _) in entries_s0 ]
            keys = argon2id_batch(pw_list, salt_list, m_cost=arg_mem, t=arg_time, p=arg_par, dk_len=32)
        else:
            keys = []
    except Exception as e:
        log_exception(e, "Batch Argon2id derivation failed; falling back to per-entry KDF")
        keys = []

    # Decrypt shares with pre-derived keys
    if keys:
        for (i, (entry, alt, _salt, q_hash, alt_hash)) in enumerate(entries_s0):
            try:
                share_bytes = _decrypt_share_from_entry(entry, arg_time, arg_mem, arg_par,
                                                        q_hash=q_hash, alt_hash=alt_hash,
                                                        qid=None, qtext=None, alt_text=alt,
                                                        pre_key=keys[i])
        ...
        # Fallback per-entry
        for (entry, alt, _salt, q_hash, alt_hash) in entries_s0:
            share_bytes = _decrypt_share_from_entry(entry, arg_time, arg_mem, arg_par,
                                                    q_hash=q_hash, alt_hash=alt_hash,
                                                    qid=None, qtext=None, alt_text=alt)
            if share_bytes is not None:
    ...
    # Try REAL path first using all shares at once (fast path when all picked are correct)
    try:
        cb_all = asyncio.run(sss_combine(partials_s0))
        b64_all = _extract_valid_b64_from_combined(cb_all)
        if b64_all:
            seed_b = base64.b64decode(b64_all.encode("utf-8"), validate=True)
            final_auth = (kit.get("config") or {}).get("final_auth")
            if (final_auth and _auth_ok_for_entry(seed_b, final_auth)) or any(
                _auth_ok_for_entry(seed_b, e) for e in auth_catalog
            ):
                final_secret_text = DTE.decode(b64_all)
                print("\n[AUTH OK]")
                print("--- SECRET RECONSTRUCTED ---")
                print(final_secret_text)
                print("-----------------------------\n")
                log_debug("Final secret reconstructed (all-shares fast path).", level="INFO", component="CRYPTO",
                          details={"final_secret_len": len(final_secret_text), "variant": "REAL", "auth_ok": True})
                return
    except Exception:
        pass

    # Try REAL path using validated subset search
    # Try REAL path first using final_auth for verification
    final_auth = (kit.get("config") or {}).get("final_auth")
    # Accept any catalog-authenticated candidate for robustness
    auth_ok = _try_combine_validated(
        partials_s0, r_thr,
        final_auth_entry=final_auth,
        auth_catalog=auth_catalog,
        prefer_final_only=False,
    )
    if auth_ok:
        variant, recovered_b64 = auth_ok
        ...
        log_debug("Final secret reconstructed.", level="INFO", component="CRYPTO",
                  details={"final_secret_len": len(final_secret_text), "variant": variant, "auth_ok": True})
        return

    # If we reach here, the reconstruction failed for the real secret.
    log_error("Authentication failed after reconstruction attempts.")
    print("\nAuthentication failed; no valid secret could be verified. Check your answers and try again.")
    return
...
def prompt_argon2_parameters():
    ...
    if choice_ == 'a':
        ...
        return (t, m_kib, p)
    if choice_ != 'e':
        print("Using FAST Argon2id parameters: time_cost=1, memory_cost=1024, parallelism=1")
        ...
        mc = get_valid_int("memory_cost KiB (1..1048576)? ", 1, 1048576)
